...
import { Code2, Crown, Sparkles} from 'lucide-react';
import Link from 'next/link';
import React from 'react'

function NavigationHeader() {
  return (
    <div className="sticky top-0 z-50 w-full border-b border-indigo-500/20 glass backdrop-blur-2xl modern-border">
        ...
        <div className="relative h-20 flex items-center justify-between">
          <div className="flex items-center gap-8">
            <Link href="/" className="flex items-center gap-4 group relative">
              {/* Modern glow effect */}
              <div className="absolute -inset-3 bg-gradient-to-r from-indigo-500/20 via-purple-500/20 to-pink-500/20 rounded-2xl opacity-0 group-hover:opacity-100 transition-all duration-500 blur-xl" />
              ...
              <div className="relative bg-gradient-to-br from-indigo-600 to-purple-600 p-3 rounded-2xl shadow-glow group-hover:shadow-glow-purple transition-all duration-300 transform group-hover:scale-110">
                <Code2 className="w-7 h-7 text-white transform group-hover:rotate-6 transition-transform duration-500" />
              </div>
